...
export default class ChartContainer extends React.PureComponent {
    ...
    this.chartWidget.onChartReady(() => {
      table.map(({ type, trade, price }) =>
        this.chartWidget
          ...
          .setQuantity(trade)
          .setPrice(price)
          .setQuantityBackgroundColor(type === 'Buy' ? '#00DA9D' : '#F33501')
          .setQuantityBorderColor(type === 'Buy' ? '#00DA9D' : '#F33501')
          .setBodyTextColor(type === 'Buy' ? '#00DA9D' : '#F33501')
          .setBodyBorderColor(type === 'Buy' ? '#00DA9D' : '#F33501')
          .setLineColor(type === 'Buy' ? '#00DA9D' : '#F33501')
      );
      this.chartWidget
        .activeChart()
        .setVisibleRange(
  ...
  widgetCreator = (props) => {
    const { selectedMarket, orderBookTable: table } = props;
